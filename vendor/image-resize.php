 ...
 * Third-Party Library
 *
 * @package Unity Framework
 * @author Unity Websoft
 * @copyright 2017 All Rights Reserved
 * @license	http://opensource.org/licenses/MIT	MIT License
 * @link http://unitywebsoft.com
 *
 */
...
class Image_Resize {
    ...
    public static function createFromString($image_data)
    {
		if(empty($image_data) || $image_data === null) {
			throw new ResizerException('image_data must not be empty');
		}
    ...
    public function __construct($filename)
    {
        ...
        if($filename === null || empty($filename) || (substr($filename,0,7) !== 'data://' && !is_file($filename))) {
			throw new ResizerException('File does not exist');
		}

		$finfo = finfo_open(FILEINFO_MIME_TYPE);
		if(strstr(finfo_file($finfo, $filename),'image') === false) {
			throw new ResizerException('Unsupported file type');
		}

        $image_info = getimagesize($filename,$this->source_info);

        if (!$image_info) {
            throw new ResizerException('Could not read file');
        }
        ...
        switch ($this->source_type) {
            ...
            default:
                throw new ResizerException('Unsupported image type');
                break;
        }

        if (!$this->source_image) {
            throw new ResizerException('Could not load image');
        }
